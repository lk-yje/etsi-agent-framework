...
class AuditFinding(BaseModel):
    ...
    # 审计意见若要参与最终裁决，不能只留在自然语言 description 中。以下字段
    # 是可机器复核的最小依据；缺失时一律只进入人工复核，绝不自动覆盖 Work 结果。
    recommended_verdict: Optional[str] = Field(
        None, alias="recommendedVerdict",
        description="verdict_wrong 时建议的 PASS|FAIL|INCONCLUSIVE|NA；无明确建议则为 null",
    )
    evidence_paths: List[str] = Field(
        default_factory=list, alias="evidencePaths",
        description="支撑建议的 IXIT/evidence JSON 路径或工作区相对路径",
    )
    override_eligible: bool = Field(
        default=False, alias="overrideEligible",
        description="仅当证据路径足以让第三方复核时为 true",
    )
