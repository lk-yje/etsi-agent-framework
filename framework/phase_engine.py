...
class PhaseExecutionEngine:
    ...
    def __init__(
        ...
        # 节点快照: 始终记录 (落盘成本低); 是否允许跳过复用由 resume 开关控制。
        self.snapshots = SnapshotStore(workspace)
        self.resume = os.environ.get("SNAPSHOT_RESUME", "") == "1"
    ...
    async def _execute_phase(
        ...
        knowledge_paths = tuple(dict.fromkeys(
            ...
            + module.ammo_paths + phase_knowledge
        ))
        ...
        task_ctx = self._build_phase_task_ctx(phase, module, pool_snapshot, pipeline)

        # 4.5 节点快照: resume 模式下输入 hash 匹配的已完成 phase 直接复用
        node_id = f"phase/{module.id}/{phase.id}"
        evidence_relpath = f"evidence/pre-{module.id}-{phase.id}-evidence.json"
        # 摘要用结构性字段而非渲染后的 task 文本 (文本含池状态提示行, 会随重跑变化);
        # 且排除本 phase 自己的 pool 输出 key —— 池索引持久化, 上次运行的输出
        # 会在重跑时出现在快照里, 算进去 digest 永远不等, 幂等跳过就失效了。
        own_output_keys = {o["key"] for o in phase.pool_outputs}
        pool_deps = {
            key: value for key, value in (pool_snapshot or {}).items()
            if key not in own_output_keys
        }
        input_digest = compute_input_digest(
            ...
            task_core={
                ...
                "output_file": task_ctx["output_file"],
            },
            pool_snapshot=pool_deps,
        ...
        if self.resume:
            reusable = self.snapshots.find_reusable(node_id, input_digest)
            if reusable is not None:
                evidence_data = json.loads(
                    (workspace / evidence_relpath).read_text(encoding="utf-8")
                )
                self._apply_pool_outputs(phase, module, evidence_data)
                self.telemetry.log_stage_event(
                    pipeline_id, "PHASE_ENGINE", "phase_snapshot_reused",
                    module=module.id, phase=phase.id,
                    attempt_id=reusable.get("attempt_id"),
                )
            ...
            try:
                ...
                if result.raw_text:
                    try:
                        ...
                        markers = conceptual_verdict_markers(candidate)
                        if markers:
                            self.telemetry.log_stage_event(
                                pipeline_id, "PHASE_ENGINE", "functional_evidence_rejected",
                                module=module.id, phase=phase.id,
        ...
        if evidence_data is None:
            ...
            # 失败也是事实: 落 failed 快照 (resume 时不跳过 failed 节点)。
            self.snapshots.record(SnapshotStore.build_snapshot(
                node_id=node_id, kind="phase", status=STATUS_FAILED,
                attempt_id=pipeline_id, input_digest=input_digest,
                model=config.model, agent_id=config.agent_id,
                parent="stage/m1_m5", started_at=started_at,
                error="all retries exhausted without valid evidence",
            ))
            self.telemetry.log_stage_event(
        ...
        self.bus.write_json(evidence_relpath, evidence_data)

        # 8. 落 completed 快照 (输入 hash + 产物 hash → 幂等跳过/证据链追溯)
        self.snapshots.record(SnapshotStore.build_snapshot(
            node_id=node_id, kind="phase", status=STATUS_COMPLETED,
            attempt_id=pipeline_id, input_digest=input_digest,
            outputs=[{
                "path": evidence_relpath,
                "digest": digest_file(workspace / evidence_relpath),
                "bytes": (workspace / evidence_relpath).stat().st_size,
            }],
            pool_outputs_written=[o["key"] for o in phase.pool_outputs],
            model=config.model, agent_id=config.agent_id,
            parent="stage/m1_m5", started_at=started_at,
        ))
    ...
    def _apply_pool_outputs(self, phase: PhaseDef, module, evidence_data: dict) -> None:
        """把 evidence 中对应的 key 写入 ContextPool (实跑与快照复用共用)。"""
    ...
    def _build_phase_task_ctx(
        ...
        task_text = (
            ...
            "不得输出概念性分析；每个 PASS/FAIL 必须有本次实测证据，未实测则写 "
            "INCONCLUSIVE 或 PENDING_MANUAL。\n"
        )
        ...
        # 工具路径由运行准备层在本次 run 中解析和校验。明确提供给 Work
        # Agent，使它调用绝对路径，而不是假定 tshark/xray 已被永久写进系统 PATH。
        resolved_tools = {}
        resolver = getattr(pipeline, "path_resolver", None)
        if resolver:
            for name in ("tshark", "xray"):
                path = resolver.get_tool_path(name)
                if path:
                    resolved_tools[name] = path
        if resolved_tools:
            task_text += "\n本次运行临时工具路径（直接使用绝对路径，勿假设系统 PATH）：\n"
            task_text += "\n".join(f"- {name}: {path}" for name, path in resolved_tools.items()) + "\n"
