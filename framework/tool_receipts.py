...
class ToolReceiptStore:
    ...
    # 进程内单调序号: Windows + Py3.12 的 datetime.now() 时钟粒度约 15.6ms,
    # 同一 tick 内的多张回据时间戳相同, 仅靠时间戳排序不严格有序。
    ...
    def record(
        ...
        *,
        ...
        receipt_id = (
            f"{started_at.strftime('%Y%m%dT%H%M%S%f')}"
            f"_{next(ToolReceiptStore._sequence):08d}_{uuid.uuid4().hex[:10]}"
        )
