    ...
    async def run_audit(
        ...
        *,
        ...
        config = AgentConfig(
            ...
            forbidden_patterns=(),
            # 与 Work Agent / PhaseEngine 一致：使用版本化的统一模型选择。
        ...
        task_ctx = {
            "task": (
                ...
                "若 finding.category=verdict_wrong，必须填写 recommendedVerdict；只有给出"
                "可复核的 evidencePaths 后才可将 overrideEligible 设为 true。没有直接证据时"
                "保持 false，不能以自然语言要求系统覆盖 Work 结论。\n"
        ...
        # AgentRunner has already extracted prose/fence-wrapped JSON and
        # validated it against AuditResult.  Re-parsing raw text here used a
        # narrower parser, turning valid compatible-model responses into
        # FLAGGED audit slices.  Prefer the validated contract output.
        if result.output:
