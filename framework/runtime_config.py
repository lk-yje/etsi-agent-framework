...
class RuntimeSettings:
    ...
    def resolve_workspace(self, value: str | Path, *, create: bool = False) -> Path:
        """Resolve a workspace and reject paths outside ``AUTO_TEST_ROOT``."""
        raw = str(value).strip()
        if not raw:
        ...
        if not target.is_relative_to(root):
            raise ValueError(f"workspace 必须位于 AUTO_TEST_ROOT 内: {root}")
        if create:
    ...
    def public_summary(self) -> dict:
        ...
        return {
            ...
            "path_mapping": {
                ...
                "source": "environment" if mapping else "framework_default",
