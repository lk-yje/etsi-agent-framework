        ...
        for clause_id in target_ids:
            ...
            if old is not None and retry_data:
                ...
                history.append({
                    ...
                    "reason": "L2 要求重新评估；Agent 仅收到干净的条款任务",
...
class TrafficCollectStage(Stage):
    ...
    async def execute(self, workspace, registry, pipeline):
        ...
        try:
            ...
            if tshark_ok:
                iface = self._detect_interface(dut_ip) if dut_ip else None
            ...
            if xray_ok:
                ...
                if xray_proc:
                    ...
                    await self._configure_burp_upstream(
            ...
            traffic_state.transition(
                ...
                channels={
                    ...
                    "burp_upstream": {"status": "configured" if xray_proc else "not_configured"},
                },
            )
            if not tshark_proc or not xray_proc:
                outcome = "BLOCKED"
                outcome_reason = "双通道未同时就绪；不得把单通道采集当作阶段 3.1 完成"
                    ...
                    if not capture_pcap.exists():
                        ...
                        if pcap_size_kb < 1:
                            ...
                            if await self._run_pcap_analyzer(
                                ...
                                {**receipt_context, "artifact_refs": ["pcap_analysis"]},
                            ):
            ...
            if outcome not in {"COMPLETE", "CANCELLED"}:
                self._mark_review(pipeline, f"Traffic {outcome}: {outcome_reason}")
    ...
    @staticmethod
    def _detect_interface(dut_ip: str) -> str | None:
        """自动检测到达 DUT 的网卡接口"""
        try:
            result = subprocess.run(
                ["route", "print", dut_ip] if os.name == "nt" else ["ip", "route", "get", dut_ip],
    ...
    @staticmethod
    async def _start_xray(workspace: Path, receipt_context: dict | None = None) -> asyncio.subprocess.Process | None:
        """启动 xray 被动扫描"""
        xray_path = shutil.which("xray") or shutil.which("xray_windows_amd64") or "xray"
        try:
            from framework.path_resolver import PathResolver
            resolver = PathResolver()
            mapped = resolver.get_tool_path("xray")
            if mapped:
                xray_path = mapped
        except Exception:
            pass
    ...
    async def _configure_burp_upstream(
        ...
        backup_path，供 _remove_burp_upstream 恢复。MCP 不可用时记录日志但不阻塞管线。
        """
        ...
        # 回退: 原始 HTTP 请求 (best-effort，非 MCP 协议端点，仅保留意图)
        ...
        try:
            ...
            payload = json.dumps({
                ...
                "arguments": {"config_json": _upstream_config(xray_servers)},
            }).encode("utf-8")
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                telemetry.log_stage_event(
                    pipeline_id, "TRAFFIC", "burp_upstream_configured",
                    target="127.0.0.1:7778", method="http_fallback",
                )
        except Exception as e:
            telemetry.log_stage_event(
...
class M1M5ParallelStage(Stage):
    ...
    async def _run_work_with_retry(
        ...
        for attempt in range(self.MAX_WORK_RETRIES + 1):
            config = AgentConfig(
                ...
                knowledge_paths=pipe_def.shared_knowledge + module.ammo_paths,
    ...
    async def _retry_work_for_clauses(
        ...
        task_text = f"请测试 {module.id} 模块以下条款：{clauses_str}"
        if recipe:
            task_text += "\n" + recipe
...
class ETSIPipeline(Pipeline):
    ...
    def __init__(self, workspace, registry, runner, bus, telemetry, pipe_def,
                 ...
                 mcp_manager=None, control_mode="terminal"):
