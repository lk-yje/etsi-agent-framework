...
def build_etsi_pipeline_def(skills_root: Path) -> PipelineDef:
    ...
    Args:
        skills_root: skills 根目录（例如项目内 skills/）
    ...
    return PipelineDef(
        ...
        shared_knowledge=(
            # M0/Audit 从完整管线知识读取；Phase Engine 会在把上下文交给
            # M1–M5 Work Agent 前应用功能性过滤。
            get_work_shared_knowledge(root) + get_conceptual_shared_knowledge(root)
        ),
