...
路径约定:
- skills_root 由 run_pipeline.py 传入（示例 C:/Users/<user>/.claude/skills）
...
# 从环境变量取 model，支持第三方 API (DeepSeek 等)
...
def get_work_shared_knowledge(skills_root: Path) -> Tuple[Path, ...]:
    """Work Agent 共享知识库（从 skills/ 目录读取）。

    Args:
        skills_root: skills 根目录，如 C:/Users/<user>/.claude/skills
    """
