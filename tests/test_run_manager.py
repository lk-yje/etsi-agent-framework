...
def test_run_manager_builds_web_command_and_allowlisted_env(monkeypatch, tmp_path):
    ...
    command = RunManager.build_command(
        workspace=tmp_path, pipeline="etsi-ts103701", mock=True, no_playwright=True,
    )
    env = RunManager.build_env(burp_token="burp-secret", burp_host="127.0.0.1", burp_port=7777)
