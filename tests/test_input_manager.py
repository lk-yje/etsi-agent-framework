...
def test_generate_tampered_from_new_verifies_sha256_changed(tmp_path):
    firmware_root = tmp_path / "firmware-library"
    firmware_root.mkdir()
    new = firmware_root / "new" / "digicap.dav"
    new.parent.mkdir()
    new.write_bytes(b"\x00" * 4096)

    manifest = generate_tampered_from_new(tmp_path, new, (firmware_root,))

    fw = manifest["inputs"]["firmware"]
    assert fw["new"]["sha256"] == hashlib.sha256(new.read_bytes()).hexdigest()
    assert fw["tampered"]["sha256_changed"] is True
    assert fw["tampered"]["generated_from_new_sha256"] == fw["new"]["sha256"]
    assert fw["tampered"]["sha256"] != fw["new"]["sha256"]

    tampered_path = firmware_root / "new" / "tampered" / "digicap_tampered.dav"
    assert tampered_path.is_file()
    assert tampered_path.read_bytes() != new.read_bytes()
    assert not (tmp_path / "inputs/firmware").exists()
