...
def test_frontend_serves_persisted_traffic_checklist_controls():
    ...
    assert 'id="inpIxitFile" placeholder="受控输入目录内的完整路径"' in response.text
    assert 'id="inpFirmwareOld" placeholder="受控固件目录内的完整路径"' in response.text
...
def test_start_run_uses_secret_env_and_preserves_ixit(tmp_path, monkeypatch):
    root = _configure_runtime(tmp_path, monkeypatch)
