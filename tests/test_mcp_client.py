...
class TestMCPClientManager:
    ...
    @pytest.mark.asyncio
    async def test_connect_burp_without_token_attempts_unauthenticated_connection(self, manager):
        """无 token 的 Burp MCP 应尝试无认证连接；失败才返回 False。"""
        async with manager:
            with patch("mcp.client.sse.sse_client", side_effect=ConnectionError("refused")) as mock_sse:
                result = await manager.connect_burp()
            mock_sse.assert_called_once_with(url="http://127.0.0.1:9876/")
    ...
    @pytest.mark.asyncio
    async def test_readonly_burp_call_reconnects_once_after_transport_close(self, manager):
        """SSE session close during a read-only call is recovered without replaying writes."""
        async with manager:
            manager._tool_cache["burp_version"] = MCPToolInfo(
                name="burp_version", description="", input_schema={}, server_name="burp",
            )
            old_session = AsyncMock()
            old_session.call_tool = AsyncMock(side_effect=RuntimeError("Connection closed"))
            new_session = AsyncMock()
            new_session.call_tool = AsyncMock(return_value=MockCallResult(
                content=[MockContentBlock(text="recovered")]
            ))
            manager._burp_session = old_session
            manager._connected["burp"] = True

            async def reconnect():
                manager._burp_session = new_session
                manager._connected["burp"] = True
                return True

            manager._reconnect_burp_once = reconnect
            assert await manager.call_tool("burp_version", {}) == "recovered"
            assert old_session.call_tool.call_count == 1
            assert new_session.call_tool.call_count == 1

    @pytest.mark.asyncio
    async def test_readonly_burp_call_uses_second_reconnect_after_first_replay_closes(self, manager):
        async with manager:
            manager._tool_cache["burp_version"] = MCPToolInfo(
                name="burp_version", description="", input_schema={}, server_name="burp",
            )
            manager._burp_session = AsyncMock()
            manager._burp_session.call_tool = AsyncMock(side_effect=RuntimeError("Connection closed"))
            manager._connected["burp"] = True
            first_recovered = AsyncMock()
            first_recovered.call_tool = AsyncMock(side_effect=RuntimeError("Connection closed"))
            second_recovered = AsyncMock()
            second_recovered.call_tool = AsyncMock(return_value=MockCallResult(
                content=[MockContentBlock(text="second recovery")]
            ))
            reconnect_count = 0
            ...
            async def reconnect():
                ...
                reconnect_count += 1
                manager._burp_session = first_recovered if reconnect_count == 1 else second_recovered
                manager._connected["burp"] = True
                return True

            manager._reconnect_burp_once = reconnect
            assert await manager.call_tool("burp_version", {}) == "second recovery"
            assert reconnect_count == 2

    def test_burp_replay_policy_only_allows_safe_reads(self, manager):
        assert manager._burp_tool_safe_to_retry("burp_version", {}) is True
        assert manager._burp_tool_safe_to_retry("http_send_request", {"method": "GET"}) is True
        assert manager._burp_tool_safe_to_retry("http_send_request", {"method": "POST"}) is False
        assert manager._burp_tool_safe_to_retry("proxy_set_options", {}) is False
    ...
    @pytest.mark.asyncio
    async def test_connect_playwright_uses_pinned_node_runtime(self, manager):
        """Playwright 必须经固定本地 cli.js 直启，不能走 npx/latest。"""
        async with manager:
            ...
            with patch("mcp.client.stdio.stdio_client") as mock_stdio, \
                ...
                # 验证 stdio_client 使用 node + 项目内固定 cli.js。
                call_args = mock_stdio.call_args[0][0]
                args_list = call_args.args
                assert call_args.command.lower().endswith(("node.exe", "node"))
                assert args_list[0].endswith("@playwright\\mcp\\cli.js") or args_list[0].endswith("@playwright/mcp/cli.js")
                assert "--isolated" in args_list
                assert all("npx" not in value and "latest" not in value for value in args_list)
