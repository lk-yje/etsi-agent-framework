...
from contracts.audit import AuditMeta, AuditResult, RetryInstruction
...
def test_m0_concept_initial_and_targeted_retry(monkeypatch, tmp_path):
    ...
    async def fake_execute(clause, workspace, pipeline):
    ...
    async def retry_execute(clause, workspace, pipeline):
        calls.append(clause.clause_id)
    ...
    class RejectingGate:
        ...
        async def run_audit(self, workspace, runner, pipe_def, **kwargs):
            return AuditResult(
                ...
                retryInstruction=RetryInstruction(targetClauseIds=["5.6-6"], memo="ignored by Work Agent"),
            )
